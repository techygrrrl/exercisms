...
class DominoesTest {
    ...
    @Test
    fun `empty input = empty output`() = Dominoes.formChain(listOf()).should { haveSize(0) }
    ...
    fun `singleton can't be chained`() {
        Dominoes.formChain(listOf(Domino(1, 2)))
    }
    ...
    fun `can't be chained`() {
        val input = listOf(Domino(1, 2), Domino(4, 1), Domino(2, 3))
        Dominoes.formChain(input)
    }

    @Test(expected = ChainNotFoundException::class)
    fun `disconnected - simple`() {
        val input = listOf(Domino(1, 1), Domino(2, 2))
        Dominoes.formChain(input)
    }

    @Test(expected = ChainNotFoundException::class)
    fun `disconnected - double loop`() {
        val input = listOf(Domino(1, 2), Domino(2, 1), Domino(3, 4), Domino(4, 3))
        Dominoes.formChain(input)
    }

    @Test(expected = ChainNotFoundException::class)
    fun `disconnected - single isolated`() {
        val input = listOf(Domino(1, 2), Domino(2, 3), Domino(3, 1), Domino(4, 4))
        Dominoes.formChain(input)
    }
