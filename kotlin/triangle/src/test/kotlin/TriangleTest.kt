...
class TriangleTest {
    ...
    @Test(expected = IllegalArgumentException::class)
    fun `triangle inequality violation - second is greater than the sum of others `() {
        Triangle(1, 3, 1)
    }
    ...
    @Test
    fun `degens - can identify a degen triangle`() {
        // degens
        assertTrue(Triangle(2, 2, 4).isDegenerate)
        assertTrue(Triangle(4, 2, 2).isDegenerate)
        assertTrue(Triangle(2, 4, 2).isDegenerate)

        // not degens
        assertFalse(Triangle(4, 4, 3).isDegenerate)
        assertFalse(Triangle(2, 2, 2).isDegenerate)
    }
}
